...
class Solution:
    def minMovesToSeat(self, seats: List[int], students: List[int]) -> int:
        ...
        print(f"{seats}")
        print(f"{students}")
        difference = [students[i] - seats[i] for i in range(len(seats))]
        return sum(abs(num) for num in difference)

sol = Solution()
seats = [3,1,5]
students = [2,7,4]
#print(sol.minMovesToSeat(seats,students)) #4
seats = [4,1,5,9]
students = [1,3,2,6]
#print(sol.minMovesToSeat(seats,students)) #7
seats = [12,14,19,19,12]
students = [19,2,17,20,7]        
print(sol.minMovesToSeat(seats,students)) # 19