Function Start-WindowsUpdate {
    ...
    if ($WUUpdates.Count -ge 1){
        ...
        if ($UpdateCount -ge 1){
            ...
            foreach ($update in $WUInstaller.Updates){
                ...
                #Display update message and progress indicator
                $progress = "[Windows Update]: Downloading update $($update.Title) - $($count) of $($UpdateCount)"
                Write-Progress -Activity "Windows Update" -Status $progress -PercentComplete (($count / $UpdateCount) * 100)
                $count++
            }
    ...
    else {Write-Host -ForegroundColor Cyan "No Updates Found"} 
}
